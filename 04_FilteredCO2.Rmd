...
* `YEAR` and `STATE_NAME` are 'bare' strings that _name_ variables, and are shorthand for `annual_co2$YEAR` etc
...
2) `geom_text(aes(label = state_label), nudge_y = 2)`. If you put the `nudge_y` inside the `aes()` you get an error ('Ignoring unknown aesthetics: nudge_y') because it's not something that can vary with the values of a variable ("not an aesthetic").
