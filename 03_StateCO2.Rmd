...
Re-open your `justlearning` project (File:Recent Projects, or 'justLearning.Rproj'). Create a new source file (File:New File:R script), to copy and paste the examples into, and save it as 'chapter 3'.
...
We want to keep things simple, so we will use a little data wrangling to get rid of the month, then save the result as an `r` dataset. This is a fairly common pattern for re-organising your data: `group_by` the relevant variables, then summarise within those groups. Here we use the `summarise_at` variant of summarise, which allows us quickly to apply a function `sum` to multiple variables. Often you want to keep your groups, but here we don't need to, so we `ungroup`. 

There will be more examples of this sort of data manipulation in Chapter TBD.

```{r}
annual_co2 <- aviation_co2 %>%
  group_by(YEAR, STATE_NAME, STATE_CODE) %>%
...
We can see that there are three numeric variables (`num`) and two character variables (`chr`). All five variables have the same number of observations (`r nrow(annual_co2)` - but maybe this is more by the time you download it). In a tibble or data.frame the columns are always the same length.
...
The third way gives a window to explore every observation. Click on the dataset name next to the blue arrow or type `View(annual_co2)` in the console (sorry about the upper case 'V') and you get a tabular data explorer, which allows you to sort and filter. You should see something like this. [Try out the sorting and filtering in the view window. Filter to show only the Netherlands, and sort by total flights.]
...
* we can specify parameters by order of appearance in the list (`x` and `y` are first and second for `aes`) or by name (`colour`), or a mix of both;
