...
(a <- c(a = 7, 3, b = 7, c = 800))
(b <- list(pi = 3.14, pie = "apple", pier = "Bournemouth", piers = c("Weston", "Wigan")))
names(b)
...
The third version is most interesting because we actually want to load data from multiple files. 

For this book, there's an admin step first, which is to get them from the web onto your machine. We do this with a list and a function that loops.

```{r}
file_segments <- c("LowCost", "TradSched", "BusAvn", "AllCargo")
urls <- paste("https://github.com/david6marsh/flights_in_R/raw/main/data/SID_Monthly_",
                file_segments, 
                ".xlsx")
local_names <- paste("data/SID_Monthly_", file_segments, ".xlsx")
